/*Mètode iteratiu de Jacobi ALEIX NIETO JUSCAFRESA*/
...
int main(void){
	...
	double a, b, epsilon, h, x, alpha, beta, sum, error, norma2=1, norma1;
	double *u0, *u1, *vecr;
	FILE *sortida;
	char sor[30];
	...
	/* / AQUÍ COMENÇA EL MÈTODE DE JACOBI / */
	...
	x = a + ((i+1)*h);
        ...
        sum = 0;
				...
				vecr[i] = h*h*(0.5)*r(x) + beta*(0.5)*(1 - (h/2)*p(x));
                        ...
                        sum =  -(0.5)*(1 - (h/2)*p(x))*u0[i+1] -(0.5)*(1 + (h/2)*p(x))*u0[i-1];
			vecr[i] = h*h*(0.5)*r(x);


                }

                u1[i] = (1/(1+((h*h)/2)*q(x)))*(vecr[i]-sum);
	}
